...
# Last Updated: 2025-02-09 17:46:24 UTC
...
if ($existingLogs) { $latestLog = $existingLogs | Sort-Object LastWriteTime -Descending | Select-Object -First 1; if ($latestLog) { Write-Host "Deleting previous log file: $($latestLog.FullName)";[...]
...
if ($existingBackups) { $latestBackup = $existingBackups | Sort-Object LastWriteTime -Descending | Select-Object -First 1; if ($latestBackup) { Write-Host "Deleting previous backup file: $($latest[...]
...
try { Write-Host "Creating backup of the current script: $currentScriptPath"; Copy-Item -Path $currentScriptPath -Destination $backupFile -Force; Write-Host "Backup created successfully: $backupFi[...]
...
} catch { Write-Log "ERROR: Failed to import required Azure modules. $_"; Stop-Transcript; exit 1 }
...
function Test-ResourceGroupExists {
    ...
    try { Get-AzResourceGroup -Name $ResourceGroupName -ErrorAction Stop | Out-Null; return $true } catch { return $false }
}
try {
    ...
    } else { Write-Log "Resource group '$resourceGroupName' exists." }
} catch { Write-Log "ERROR: Failed to verify or create resource group. $_"; Stop-Transcript; exit 1 }
try {
    ...
    if ($existingStorage) { Write-Log "Storage account '$storageAccountName' already exists. Removing..."; Remove-AzStorageAccount -Name $storageAccountName -ResourceGroupName $resourceGroupName -[...]
...
} catch { Write-Log "ERROR: Failed to verify or create storage account. $_"; Stop-Transcript; exit 1 }
try {
    ...
    if ($existingNsg) { Write-Log "NSG '$nsgName' already exists. Removing..."; Remove-AzNetworkSecurityGroup -Name $nsgName -ResourceGroupName $resourceGroupName -Force -Confirm:$false; Write-Log[...]
    ...
    $denyRule = New-AzNetworkSecurityRuleConfig -Name "Deny-RDP-3389" -Protocol Tcp -Direction Inbound -Priority 900 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -Destinat[...]
    $allowRule = New-AzNetworkSecurityRuleConfig -Name "Allow-RDP-10443" -Protocol Tcp -Direction Inbound -Priority 1000 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -Dest[...]
...
try {
    ...
    if ($existingPublicIp) { Write-Log "Public IP '$publicIpName' already exists. Removing..."; Remove-AzPublicIpAddress -Name $publicIpName -ResourceGroupName $resourceGroupName -Force -Confirm:[...]
...
} catch { Write-Log "ERROR: Failed to create Public IP. $_"; Stop-Transcript; exit 1 }
try {
    ...
    if ($existingVnet) { Write-Log "Virtual Network '$vnetName' already exists. Removing..."; Remove-AzVirtualNetwork -Name $vnetName -ResourceGroupName $resourceGroupName -Force -Confirm:$false;[...]
...
} catch { Write-Log "ERROR: Failed to verify or create Virtual Network. $_"; Stop-Transcript; exit 1 }
try {
    $nicName = "$vmName-NIC"
    ...
    if ($existingNic) { Write-Log "Network Interface '$nicName' already exists. Removing..."; Remove-AzNetworkInterface -Name $nicName -ResourceGroupName $resourceGroupName -Force -Confirm:$false[...]
    Write-Log "Creating Network Interface for VM '$vmName' and associating NSG '$nsgName'..."
    $subnet = $vnet.Subnets | Where-Object { $_.Name -eq $subnetName }
    if (-not $subnet) { throw "Subnet '$subnetName' could not be found in Virtual Network '$vnetName'." }
    $nic = New-AzNetworkInterface -Name $nicName -ResourceGroupName $resourceGroupName -Location $location -SubnetId $subnet.Id -PublicIpAddressId $publicIp.Id -NetworkSecurityGroupId $nsg.Id -Er[...]
    Write-Log "Network Interface for VM '$vmName' created."
} catch { Write-Log "ERROR: Failed to create Network Interface. $_"; Stop-Transcript; exit 1 }
try {
    ...
    if ($existingVm) { Write-Log "Virtual Machine '$vmName' already exists. Removing..."; Remove-AzVM -Name $vmName -ResourceGroupName $resourceGroupName -Force -Confirm:$false; Write-Log "Virtua[...]
